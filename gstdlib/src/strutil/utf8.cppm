...
export namespace utf8 {
	...
	inline bool valid_lead_byte(char first_byte) noexcept {
		int const count = std::countl_one(static_cast<uint8>(first_byte));
		return count != 1 && count <= 4;
	}
	
	// Get the byte length of a UTF-8 character from its first byte
    inline int64 char_len(char first_byte) noexcept {
        return std::max(1, std::countl_one(static_cast<uint8>(first_byte)));
    }

    // Count the number of UTF-8 characters in a byte range
    inline int64 count_chars(const char* data, int64 byte_len) {
        int64 char_count = 0;
        int64 i = 0;
        while (i < byte_len) {
            i += char_len(data[i]);
    ...
    inline int64 length(const char* data) {
        int64 char_count = 0;
        int64 i = 0;
        while (data[i] != 0) {
            i += char_len(data[i]);
    ...
    inline int64 byte_length(const char* data) {
        int64 i = 0;
        while (data[i] != 0) {
            i += 1;
        }
    ...
    inline int64 char_index_to_byte_offset(const char* data, int64 byte_len, int64 char_idx) {
        int64 byte_offset = 0;
        int64 char_count = 0;
    ...
    inline int64 get_char_byte_len(const char* data, int64 byte_offset, int64 byte_len) {
        if (byte_offset >= byte_len) return 0;
        return char_len(data[byte_offset]);
    }

	// Convert utf-8 byte data to a char32_t
	inline char32_t utf8_to_char32(const char* data, int64 byte_offset, int64 char_byte_len) noexcept {
	...
	inline int64 find_last_char_start(const char* data, int64 byte_len) {
	...
	inline constexpr int64 char_len_from_lead(char first_byte) noexcept {
		auto const b = static_cast<uint8>(first_byte);
#if !defined(NDEBUG)
...
#endif
		return std::max(1ll, static_cast<int64>(std::countl_one(b)));
	}

	// Count character positions in a byte range. Assumes valid UTF-8.
	[[nodiscard]] inline int64 count_chars(std::span<const char> data) noexcept {
		int64 chars = 0;
		for (auto const c : data) {
			chars++;
			int64 const len = char_len_from_lead(c);
			data = data.subspan(len > std::ssize(data) ? 1uz : static_cast<std::size_t>(len));
		}
		return chars;
	}

	// Convert character index to byte offset. Clamps to buffer end if idx out-of-bounds.
	[[nodiscard]] inline int64 char_index_to_byte_offset(std::span<const char> data, int64 char_idx) noexcept {
		if (char_idx < 0) return 0;
		int64 offset = 0;
		for (int64 i = 0; i < char_idx && offset < std::ssize(data); ++i) {
			auto const len = char_len_from_lead(data[offset]);
			offset += std::min(len, static_cast<int64>(std::ssize(data)) - offset);
		}
		return offset;
	}

	// Decode a code point at byte_offset. Updates offset forward. Returns U+FFFD on error.
	[[nodiscard]] inline uint32 decode_code_point(std::span<const char> data, int64& offset) noexcept {
		if (offset < 0 || offset >= std::ssize(data)) return 0xFFFD; // replacement char

		auto const b = static_cast<uint8>(data[offset]);
		uint32 cp = 0;
		int64 len = 1;
		...
		if ((b & 0x80) == 0x00) cp = b;
			...
			cp = static_cast<uint32>((b & 0x1F) << 6 | (static_cast<uint8>(data[offset + 1]) & 0x3F));
			len = 2;
		}
		else if ((b & 0xF0) == 0xE0) {
			if (offset + 2 >= std::ssize(data)) return 0xFFFD;
			cp = static_cast<uint32>((b & 0x0F) << 12 | (static_cast<uint8>(data[offset + 1]) & 0x3F) << 6 | (static_cast<uint8>(data[offset + 2]) & 0x3F));
			len = 3;
		}
		else if ((b & 0xF8) == 0xF0) {
			if (offset + 3 >= std::ssize(data)) return 0xFFFD;
			cp = static_cast<uint32>((b & 0x07) << 18 | (static_cast<uint8>(data[offset + 1]) & 0x3F) << 12 |
				(static_cast<uint8>(data[offset + 2]) & 0x3F) << 6 | (static_cast<uint8>(data[offset + 3]) & 0x3F));
			len = 4;
		}
		else {
	...
	[[nodiscard]] inline int64 encode_code_point(uint32 cp, char* const buf, int64 capacity) noexcept {
	...
	[[nodiscard]] inline bool validate(std::span<const char> data) noexcept {
		for (; !data.empty(); ) {
			auto const b = static_cast<uint8>(data[0]);
			if (b == 0x00) return true; // valid end-of-string

			int64 expected_len = 1;
			if ((b & 0x80)) {
				...
				if (std::ssize(data) < expected_len) return false;
				for (int64 i = 1; i < expected_len; ++i) {
					if ((static_cast<uint8>(data[i]) & 0xC0) != 0x80) return false; // malformed continuation
				}
			}
