...
export namespace http {
	string get(string url) {
		...
		HINTERNET const hRequest = WinHttpOpenRequest(hConnect, L"GET", path.c_str(),
		...
		if (!WinHttpSendRequest(hRequest, WINHTTP_NO_ADDITIONAL_HEADERS, 0,
			...
			WinHttpCloseHandle(hConnect);
			WinHttpCloseHandle(hSession);
			throw std::system_error(err, std::system_category(), "WinHttpSendRequest failed");
		}
		...
		if (!WinHttpReceiveResponse(hRequest, nullptr)) {
			...
			WinHttpCloseHandle(hConnect);
			WinHttpCloseHandle(hSession);
			throw std::system_error(err, std::system_category(), "WinHttpReceiveResponse failed");
		}
